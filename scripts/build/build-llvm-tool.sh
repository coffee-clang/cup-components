...
llvm_compiler_rt_darwin_cmake_args() {
    ...
    local native_arch
    native_arch="$(macos_native_arch)"

    args+=(
        -DCOMPILER_RT_ENABLE_IOS:BOOL=OFF
        -DCOMPILER_RT_ENABLE_WATCHOS:BOOL=OFF
        -DCOMPILER_RT_ENABLE_TVOS:BOOL=OFF
        "-DDARWIN_osx_ARCHS:STRING=$native_arch"
        "-DDARWIN_osx_BUILTIN_ARCHS:STRING=$native_arch"
    )
...
prune_unowned_clang_runtime_payload() {
    ...
    find "$PREFIX" -type f -name '*clang_rt*' \
        \( -name '*profile_rocm*' \
           ...
           -o -name '*lsan*' \
           -o -name '*stats*' \
           -o -name '*ubsan_loop_detect*' \) -delete
    find "$PREFIX" -type f -name '*c++experimental*' -delete
